...
const BidRequest = () => {
  ...
  const { mutateAsync } = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('data is updateed')
      refetch()
    }
  });

  const handleStatus = async (id, prevStatus, status) => {
    if (prevStatus === status) return console.log("hobe na reeeeeeeeeeeeee");

   await mutateAsync({ id, status });
  };
  if (isLoading)
    return <p className="text-4xl text-center"> Data is Loading</p>;
  return (
